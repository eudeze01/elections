...
function Home() {
    ...
    const [pollingCentreData, setPollingCentreData] = useState([]);

    const pollingCentre = electionData?.electionPollingCentre;
    const ward = electionData?.electionWard;
    const lga = electionData?.electionLga;
    const state = electionData?.electionState;
    const federal = electionData?.electionFederal;
    const totalVotes = electionData?.electionVotes?.voteTotal;
    const votesParties = electionData?.electionVotes?.votesParties || [];
    ...
    useEffect(() => {
        // Fetch state data
        fetch('https://9656mgkl5a.execute-api.eu-west-2.amazonaws.com/dev/fetch/document/State')
            .then(response => response.json())
            .then(data => setStateData(data))
            .catch(err => console.error("Error fetching state data:", err));
    
        // Fetch LGA data
        fetch('https://9656mgkl5a.execute-api.eu-west-2.amazonaws.com/dev/fetch/document/LGA')
            .then(response => response.json())
            .then(data => setLgaData(data))
            .catch(err => console.error("Error fetching LGA data:", err));
    
        // Fetch ward data
        fetch('https://9656mgkl5a.execute-api.eu-west-2.amazonaws.com/dev/fetch/document/Ward')
            .then(response => response.json())
            .then(data => setWardData(data))
            .catch(err => console.error("Error fetching ward data:", err));
    
        // Fetch polling centre data
        fetch('https://9656mgkl5a.execute-api.eu-west-2.amazonaws.com/dev/fetch/document/ElectionResults')
            .then(response => response.json())
            .then(data => setPollingCentreData(data))
            .catch(err => console.error("Error fetching polling centre data:", err));
    }, []);
        ...
        <Box className="box">
            ...
            <Grid container spacing={3}>
                <Grid item xs={12} md={6}>
                    <Card className="card"> 
                    <StateChart data={state} votes={votesParties} />
                    </Card>
                </Grid>
                <Grid item xs={12} md={6}>
                    <Card className="card">
                    <LGAChart data={lga} votes={votesParties} />
                    </Card>
                </Grid>
                <Grid item xs={12} md={6}>
                    <Card className="card">
                    <WardChart data={ward} votes={votesParties} />
                    </Card>
                </Grid>
                <Grid item xs={12} md={6}>
                    <Card className="card">
                    <PollingCentreChart data={pollingCentre} votes={votesParties} />
                    </Card>
                </Grid>
            </Grid>
        </Box>
